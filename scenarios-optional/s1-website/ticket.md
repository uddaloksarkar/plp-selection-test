...
**Raised by:** Head, Statistics Laboratory · **Priority:** High · **Host:** this machine

> "Since this morning http://statlab.isi.local/ shows *site can't be reached* from
...
The departmental web server is not serving the Statistics Laboratory site. The
service is not running, and it does not come back on its own.

## 2. How to reproduce it

```bash
curl -I -H 'Host: statlab.isi.local' http://127.0.0.1/
```
...
1. `curl -H 'Host: statlab.isi.local' http://127.0.0.1/` returns the department
   page. It contains the marker `STATLAB-DEPT-PAGE`.
2. `curl -o /dev/null -w '%{http_code}' -H 'Host: statlab.isi.local' http://127.0.0.1/reports/`
   returns `200`.
3. The site comes back **automatically after a reboot**.

## Constraints — these are marked

- Do not change the contents of any page under `/srv/www/statlab`.
